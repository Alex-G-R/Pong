export function easy() {
    ...
    opponentSpeed = 3; // Set speed of the opponent
    ...
    function printScore() {
        ...
        context.fillText(`Easy bot: ${opponentScore}`, board.width - 200, 30);
    }
    ...
    function increaseBallSpeed() {
        ...
        if (Math.sign(ballVelocityX) === -1) {
            ballVelocityX = ballVelocityX - 0.2; // Set a fixed value
        } else {
            ballVelocityX = ballVelocityX + 0.2; // Set a fixed value
        }

        if (Math.sign(ballVelocityY) === -1) {
            ballVelocityY = ballVelocityY - 0.2; // Set a fixed value
        } else {
            ballVelocityY = ballVelocityY + 0.2; // Set a fixed value
        }
    ...
    function checkIfGameEnded() {
        if(playerScore == 3){
            alert("You won with easy bot! Congratulations!");
            ...
            alert("You lost with easy bot! Try again!");
            clearInterval(updateInterval);
            board.style.display = "none";
            pvpMode.style.display = "block";
            campainMode.style.display = "block";
