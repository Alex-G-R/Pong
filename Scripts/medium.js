...
mediumBtn.addEventListener("click", e => {
    opponentSpeed = 5.5; // Set speed of the opponent as fixed value
...
function printScore() {
    ...
    context.fillText(`Medium bot: ${opponentScore}`, board.width - 200, 30);
}
...
function increaseBallSpeed() {
    ...
    if (Math.sign(ballVelocityX) === -1) {
        ballVelocityX = ballVelocityX - 0.25; // Set a fixed value
    } else {
        ballVelocityX = ballVelocityX + 0.25; // Set a fixed value
    }

    if (Math.sign(ballVelocityY) === -1) {
        ballVelocityY = ballVelocityY - 0.25; // Set a fixed value
    } else {
        ballVelocityY = ballVelocityY + 0.25; // Set a fixed value
    }
...
function checkIfGameEnded() {
    if(playerScore == 3){
        alert("You won with medium bot! Congratulations!")
        location.reload();
    } else if (opponentScore == 3){
        location.reload();
        alert("You lost with medium bot! Try again!")
    }
}
